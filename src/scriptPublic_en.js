...
function renderTable(parentElementId, dataArray, columnNumberToSort, isSortAscending = true, isFullTable = true, lastRow = 0) {
    ...
    html += `<input type="button" class="clipboard" value="Copy table" data-clipboard-target="#${parentElementId}Table" onclick="clearSelection();">`;
    if (parentElementId === 'quantitativeValues') {
        html += `
        <table id="quantitativeSetup" class="quantitativeSetup">
            <tr>
                <td>digital capacity for min. "Click share" value</td>
                <td><div>
                ...
                <td>max. permissible error in % for "Click share"</td>
                <td><div>
                ...
                <td>digital capacity for min. "Conversion share" value</td>
                <td><div>
                ...
                <td>max. permissible error in % for "Conversion share"</td>
                <td><div>
        ...
        if (isFullTable) {
            ...
            for (let j = 1; j < dataArray[0].length; j++) {
                ...
                if (columnName.isClick && parentElementId === 'quantitativeValues') {
                    html += `<td title='${
                        dataArray[i][5]} : ${dataArray[i][9]} : ${dataArray[i][13]} : ${analyse.quantitativeClicksOther[i]} ( first : second : third : other )\n${
                        (analyse.quantitativeAccuracyClick[i] * 100).toFixed(3)}% - computed permissible error, is limited to:\n${
                        (analyse.accuracyClick * 100).toFixed(3)}% - maximum permissible error, or\n${
                        analyse.capacityClick} - digital capacity of minimall value'>`;
                } else if (columnName.isConversion && parentElementId === 'quantitativeValues') {
                    html += `<td title='${
                        dataArray[i][6]} : ${dataArray[i][10]} : ${dataArray[i][14]} : ${analyse.quantitativeConversionsOther[i]} ( first : second : third : other )\n${
                        (analyse.quantitativeAccuracyConversion[i] * 100).toFixed(3)}% - computed permissible error, is limited to:\n${
                        (analyse.accuracyConversion * 100).toFixed(3)}% - maximum permissible error, or\n${
                        analyse.capacityClick} - digital capacity of minimall value'>`;
                } else html += '<td>';
...
function validateParsingResult(result) {
    ...
    if (result.data.length < 3) {
        errorMessage = 'A minimum 3 rows expected:\n1 - service row\n2 - row with columns headers\n3 - data row';
    } else if (result.data[1].length !== 15) {
        errorMessage = 'A 15 columns expected';
    } else if (columnHeaders.toString() !== result.data[1].toString()) {
        errorMessage = `\ncolumns headers expected in the second row: \n${columnHeaders.join('\n')}`;
    }

    if (errorMessage) {
        analyse.error = true;
        analyse.errorMsg = `Import error: ${errorMessage}`;
        // eslint-disable-next-line no-alert
...
function tableClick(event) {
    if (event.target.closest('td')
        ...
        html = `<h3>1. Competitors ASINs '${analyse.selectedAsins.join("', '")}' frequency by search term: "${analyse.searchTerm}"
                    <input type="button" onclick="clearFilter()" value="clear filter">
                </h3>`;
        document.getElementById('competitorsHeader').innerHTML = html;

        html = `<h3>2. Filtered by competitors ASINs '${analyse.selectedAsins.join("', '")}' with search term: "${analyse.searchTerm}"  <input type="button" onclick="clearFilter()" value="clear filter"></h3>`;
        document.getElementById('frequencyHeader').innerHTML = html;

        // eslint-disable-next-line max-len
        html = '<h3>3. Quantitative values for "Click Share" and "Conversion Share" fields <input type="button" onclick="clearFilter()" value="clear filter"></h3>';
        document.getElementById('quantitativeValuesHeader').innerHTML = html;

        renderTable(
            'competitors',
            analyse.competitors,
            analyse.competitorsTableHeaders.indexOf('Occurrence frequency') !== -1 ? analyse.competitorsTableHeaders.indexOf('Occurrence frequency') : 0,
            false,
            false,
        );
...
function createQuantitativeTitle(rowIndex, columnType) {
    ...
    return `${
        analyse.quantitativeValues[rowIndex][isClick ? 5 : 6]} : ${analyse.quantitativeValues[rowIndex][isClick ? 9 : 10]} : ${analyse.quantitativeValues[rowIndex][isClick ? 13 : 14]} : ${analyse[isClick ? 'quantitativeClicksOther' : 'quantitativeConversionsOther'][rowIndex]} ( first : second : third : other )\n${
        (analyse[isClick ? 'quantitativeAccuracyClick' : 'quantitativeAccuracyConversion'][rowIndex] * 100).toFixed(3)}% - computed permissible error, is limited to:\n${
        (analyse[isClick ? 'accuracyClick' : 'accuracyConversion'] * 100).toFixed(3)}% - maximum permissible error, or\n${
        analyse[isClick ? 'capacityClick' : 'capacityConversion']} - digital capacity of minimall value`;
}
